...
and pauses the auto-loop scrapers to prevent CPU/GPU contention with the
game. Sentinels (LBC/PAP) keep running regardless — only the auto-loop
cycles are gated.
...
Windows-only (uses pywin32). On other platforms `is_paused()` always
returns False, so the bot behaves normally.

The previous user-active pause (paused scrapers when keyboard/mouse input
was recent) was removed 2026-05-21 — it held the auto-loop hostage to
typing activity, including chatting with Claude, and was not delivering
the intended responsiveness benefit.
"""
...
def get_state() -> dict:
    ...
    return {
        "paused": paused,
        "paused_reason": "game" if _paused_game else None,
...
async def watch_loop(notify_pause=None, notify_resume=None) -> None:
    ...
    (async callables) on game-pause transitions, e.g. to send a Telegram
    message.
    """
    global _paused_game, _pause_started_at, _last_state_change
    ...
    while True:
        try:
            ...
            if _paused_game != was_paused:
                _last_state_change = now
                if _paused_game:
                    _pause_started_at = now
                    if notify_pause:
                        try:
                            await notify_pause("game")
                        except Exception:
                            pass
                else:
